...
<div class="row {{ $ROW_CLASS ?? '' }}">
    <div class="col">
        <div class="card shadow mb-4">
                    ...
                    <div class="dropdown no-arrow">
                        ...
                        <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                            <div class="dropdown-header">
                                {{ __('messages.components.avaliationReport.downloadHeader') }}
                            </div>

                            @if ($RepExportFeature->validate())
                                <a class="dropdown-item" target="_blank"
                                    href="{{ route('app.report.pdf', ['reportClass' => class_basename($reportClassName)]) }}">
                                    {!! $Icons::FILE_PDF !!}&nbsp;
                                    {{ __('messages.components.avaliationReport.downloadPdfButton') }}
                                </a>

                                <a class="dropdown-item" target="_blank"
                                    href="{{ route('app.report.csv', ['reportClass' => class_basename($reportClassName)]) }}">
                                    {!! $Icons::FILE_CSV !!}&nbsp;
            ...
            <div class="card-body">
                {{-- Descrição com visual adaptado --}}
                <p>
                    {{ $REPORT->getDescription() }}
                </p>

                {{-- Conteúdo: Livewire ou HTML estático --}}
                <livewire:table
