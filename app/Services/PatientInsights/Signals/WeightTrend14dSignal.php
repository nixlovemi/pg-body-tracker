...
class WeightTrend14dSignal extends AbstractSignal
{
    public function key(): string
    {
        return 'weight_trend_14d';
    }
    ...
    {
        $avaliations = $context->avaliationsWithinDays(14);
        ...
        if (!$goal || $goal->objective === Goal::OBJECTIVE_HEALTH) {
            return $this->result(
                ...
                round($pctChange, 2),
                ['percent_change' => round($pctChange, 2), 'window_days' => 14]
            );
        }

        $delta = (float) $this->cfg('neutral_margin_percent', 0.3);
